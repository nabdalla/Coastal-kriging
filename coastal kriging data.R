...
curved<-data.frame((sortthc2$log))
variod <- variog(coords = cbind(sortthc2$tthc, rep(0,60)), data = curved, breaks=seq(0,0.00012,l=3))
fitd <- variofit(variod, ini.cov.pars = initial.values,  cov.model = "exponential")
...
tauthc.post<-(exact1thc$p.samples[,3])
...
for(i in 1:n.samples){
  v2thc[[i]]<-sigmathc.post[i]*exp(-phithc*dist2thc)+tauthc.post[i]*diag(n2+n1)
...
for(j in 1:(0.5*n.samples)){
  ...
  vd2b[[j]]<-sigmathc.post[j]*exp(-phithc*dist2thc[1:n1,1:n1])+tauthc.post[j]*diag(n1)
...
MyMapthc <- GetMap.bbox(lonR = c(-89.38,-89.35), latR = c(30,30.35),  
                        size=c(640,640), maptype = "hybrid")
#We can also obtain the posterior means and standard deviations of the spatial residuals and plot on the map.

w.hat.mu1 <- apply(sp1,1,mean)
w.hat.sd1 <- apply(sp1,1,sd)
...
legend("topleft",fill=attr(quant.colthc,"palette"),legend=
         names(attr(quant.colthc,"table")),bg="white", cex=1, bty="n")
plot(radius*sortthc2$x[trainthc],radius*sortthc2$y[trainthc], col=quant.colthc, cex=0.5,ylab="Northing", xlab="Easting")
mtext(text="Easting",side=1,line=2)
