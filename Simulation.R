...
fit <- variofit(vario, ini.cov.pars = initial.values,  cov.model = "wave")
...
fits <- variofit(varios, ini.cov.pars = initial.values,  cov.model = "wave")
...
dist<-as.matrix(dist(data.frame(cbind(x[c(train,test)],y[c(train,test)])),upper=TRUE, diag=TRUE,method="euclidean"))
v<-vv.p<-muhat<-yhat2<-uline<-lline<-vector("list")
for(i in 1:n.samples){
...
priors.1 <- list("beta.Norm"=list(rep(0,p), diag(1000,p)),
                 "phi.Unif"=c(0.1, 3/0.1), "sigma.sq.IG"=c(2,2),
                 "tau.sq.IG"=c(2, 2))
#model 1a
...
plotCI(z[test], predsimsk,ui=uppersimsk, li=lowersimsk,xlab="True", ylab="Predicted",
       ...
       cex.main=0.9,cex=0.9)
abline(0,1)
round(summary(msim1$p.theta.recover.samples)$quantiles,3)
round(summary(msim1$p.beta.recover.samples)$quantiles,3)